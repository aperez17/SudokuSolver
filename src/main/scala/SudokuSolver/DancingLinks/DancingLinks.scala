object DancingLinks extends SudokuSolver {
  def solve(puzzle: Puzzle):Seq[Seq[Option[Int]]] = {
    val links = new DancingLinks(puzzle.board.size)
    links.solve(puzzle)
  }
}

class DancingLinks(size: Int) {
  lazy val parts = Math.sqrt(size).toInt
  
  def solve(puzzle: Puzzle): Seq[Seq[Option[Int]]] = {
    val matrix = convertPuzzleToConstraintMatrix(Puzzle.convertBoardToNumbers(puzzle.board))
    val result = AlgorithmX.solve(matrix)
    if(result.success)
    {
      convertSolvedMatrixToPuzzle(result.rowIds).map(_.map(Some(_)).toSeq).toSeq
    }
    else {
  ...
  private def convertPuzzleToConstraintMatrix(puzzle : Seq[Seq[Int]]) = {
